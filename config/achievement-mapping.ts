...
export function guessGameFromAchievement(
  ...
  if (appId === 2552430) {
    if (/sora|riku|card (master|collection|breaker|deck)|castle oblivion|d report|veggie|balloon glider|tigger|whirlwind plunge|bumble|moogle|room creator|duel master|sleight|woven memories|truth.*castle|confronting the darkness|challenge from the inner|overcoming the darkness|card of memories|slumber to remember|road to dawn|no escape|premium card|novice deck|expert deck|interceptor|tightrope|battle card collector/i.test(name + desc) || /^com_|^rec_|rechain|chain of memories/i.test(api))
      return "Re:Chain of Memories";
    if (/terra|ventus|aqua|unversed|mirage arena|command board|vanitas|ice cream beat|shotlock|rumble racing|fruitball|eraqus|no heart|secret episode|final episode|shot lock|break time|the adventurer|the warrior|the vessel|the dormant|the seeker|musclehead|b-boy|majorette|the lost chapter|connected.*hearts|pawn of the arena|knight of the arena|queen of the arena|king of the arena|keepers of the arena|villains' vendetta|pâtissier|maestro|rapid racer|fantasista|vanitas remnant|unknown|light's will|darkness's will/i.test(name + desc) || /^bbs_|^bbs-/i.test(api))
      return "Birth by Sleep Final Mix";
    if (/roxas|summer's end|lingering will|organization xiii|paradox cup|pain and panic|cerberus paradox|titan paradox|hades paradox|goddess of fate|land of dragons|beast's castle|port royal|pride lands|space paranoids|mushroom xiii|reunion|my hero|corroded by darkness|timeless river|ambitious adventurer|nobody know-it-all|heartless highbrow|limit master|craftsman|seeker|minigame maniac|puzzler|navigator|conqueror|struggle champion|pro skater|to rule them all/i.test(name + desc) || /^kh2_|^kh2-|^khii/i.test(api))
  ...
  if (appId === 2552450) {
    if (/re mind|re:mind|limitcut|yozora|data greeting|remind|re\\s*mind/i.test(name + desc) || /remind|re_mind|limitcut|yozora|dlc/i.test(api))
      return "Re Mind (DLC)";
